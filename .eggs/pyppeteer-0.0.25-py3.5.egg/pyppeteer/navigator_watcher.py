...
class NavigatorWatcher():
    ...
    def __init__(self, frameManager: FrameManager, frame: Frame, timeout: int, options: Dict = None, **kwargs: Any) -> None:
        ...
        self._eventListeners = [helper.addEventListener(self._frameManager, FrameManager.Events.LifecycleEvent, self._checkLifecycleComplete), helper.addEventListener(
            self._frameManager, FrameManager.Events.FrameNavigatedWithinDocument, self._navigatedWithinDocument), helper.addEventListener(self._frameManager, FrameManager.Events.FrameDetached, self._checkLifecycleComplete)]
    ...
    def _validate_options(self, options: Dict) -> None:
        ...
        if isinstance(_waitUntil, list):
            ...
            raise TypeError(''.join(
                ['`waitUntil` option should be str or list of str, but got type ', '{}'.format(type(_waitUntil))]))
