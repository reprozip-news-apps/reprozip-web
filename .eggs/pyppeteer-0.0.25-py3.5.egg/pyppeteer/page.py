...
class Page(EventEmitter):
    ...
    Events = SimpleNamespace(Close='close', Console='console', Dialog='dialog', DOMContentLoaded='domcontentloaded', Error='error', PageError='pageerror', Request='request', Response='response', RequestFailed='requestfailed',
                             RequestFinished='requestfinished', FrameAttached='frameattached', FrameDetached='framedetached', FrameNavigated='framenavigated', Load='load', Metrics='metrics', WorkerCreated='workercreated', WorkerDestroyed='workerdestroyed')
    ...
    async def create(client: CDPSession, target: 'Target', ignoreHTTPSErrors: bool, setDefaultViewport: bool, screenshotTaskQueue: list = None) -> 'Page':
        ...
        (await asyncio.gather(client.send('Target.setAutoAttach', {
            'autoAttach': True,
    ...
    def __init__(self, client: CDPSession, target: 'Target', frameTree: Dict, ignoreHTTPSErrors: bool, screenshotTaskQueue: list = None) -> None:
        super().__init__()
        self._closed = False
        self._client = client
        self._target = target
        ...
        def _onTargetAttached(event: Dict) -> None:
            ...
            worker = Worker(
                session, targetInfo['url'], self._addConsoleMessage, self._handleException)
    ...
    def _onCertificateError(self, event: Any) -> None:
        if (not self._ignoreHTTPSErrors):
            return
        self._client._loop.create_task(self._send('Security.handleCertificateError', {
    ...
    async def exposeFunction(self, name: str, pyppeteerFunction: Callable[(..., Any)]) -> None:
        ...
        (await self._client.send('Runtime.addBinding', {
            'name': name,
        }))
    ...
    async def _navigate(self, url: str, referrer: str) -> Optional[str]:
        ...
        if response.get('errorText'):
            return ''.join(['{}'.format(response['errorText']), ' at ', '{}'.format(url)])
        return None
    ...
    async def setBypassCSP(self, enabled: bool) -> None:
        ...
        (await self._client.send('Page.setBypassCSP', {
            'enabled': enabled,
        }))
...
async def craete(*args: Any, **kwargs: Any) -> Page:
    ...
    logger.warning(
        '`craete` function is deprecated and will be removed in future. Use `Page.create` instead.')
